...
const initMp3MediaEncoder = ({vmsgWasmUrl})=>{
  ...
  function getWasmModuleFallback(url, imports) {
      return fetch(url).then(response=>response.arrayBuffer()).then(buffer=>WebAssembly.instantiate(buffer, imports));
  }
